...
func createFile(absFilename string, fileStorePath string, w http.ResponseWriter, r *http.Request) error {
	...
	if err != nil {
		...
		return fmt.Errorf("failed to create directory %s", absDirectory)
	}
	...
	if err != nil {
		...
		return fmt.Errorf("failed to create file %s", absFilename)
	}
	...
	if err != nil {
		http.Error(w, "Internal server error", http.StatusInternalServerError)
		return fmt.Errorf("failed to copy file contents to %s", absFilename)
	}
